...
public interface OrderMybatisMapper {
    ...
    @Select("SELECT place_number FROM places WHERE id_trip_date = #{idTripDate} AND id_passenger IS NULL")
    List<Integer> getFreePlaces(@Param("idTripDate") int idTripDate);

    @Update("UPDATE places SET id_passenger = #{place.passenger.id}" +
            " WHERE place_number = #{place.placeNumber} AND id_trip_date = #{idTripDate} AND id_passenger IS NULL")
    int changePlace(@Param("place") Place place, @Param("idTripDate") int idTripDate);
    ...
    int takePlace(@Param("idTripDate") int idTripDate, @Param("place") Place place);
    ...
    int takePlaces(@Param("passengersNumber") int passengersNumber, @Param("order") Order order);

    @Update("UPDATE places SET id_passenger = NULL WHERE id_passenger = #{place.passenger.id} AND id_trip_date = #{idTripDate}")
    void freePlace(@Param("place") Place place, @Param("idTripDate") int idTripDate);
}
